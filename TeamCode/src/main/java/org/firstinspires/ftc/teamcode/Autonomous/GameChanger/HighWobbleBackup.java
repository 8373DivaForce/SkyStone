...
public class HighWobbleBackup implements autoBase {
    ...
    public HighWobbleBackup(LinearOpMode opMode) {
        this.opMode = opMode;
    ...
    public static class waitForSpeedUp implements task {
        ...
        public int loop(RobotConstructor robot) {
            ...
            if (Math.abs(((DcMotorEx) HighWobbleBackup.robot.shooter).getVelocity()) > robotConstants.shooterSpeed-100) return -1;
    ...
    public static class runTillSpeedDown implements task {
        ...
        public void init(RobotConstructor robot) {
            //set the start time for the timeout and set the motors to run using encoders
            startTime = System.currentTimeMillis();
            HighWobbleBackup.robot.magazine.setPower(0);
            HighWobbleBackup.robot.magazine.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        }

        @Override
        public int loop(RobotConstructor robot) {
            HighWobbleBackup.robot.magazine.setPower(0.75);
            //check if the program has ran for too long, if so, terminate it
            if (System.currentTimeMillis()-startTime >= timeOut) return -2;
            //iterate through each motor
            if (Math.abs(((DcMotorEx) HighWobbleBackup.robot.shooter).getVelocity()) < robotConstants.shooterSpeed-shooterSpeed) {
                HighWobbleBackup.robot.magazine.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                HighWobbleBackup.robot.magazine.setTargetPosition(0);
                HighWobbleBackup.robot.magazine.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                HighWobbleBackup.robot.magazine.setPower(1);
                return -1;
            }
            //return that the program is still running if it has gotten to this point
            return 1;
    ...
    public static class stopShooter implements task {
        ...
        public void init(RobotConstructor robot) {
            //set the start time for the timeout and set the motors to run using encoders
            HighWobbleBackup.robot.shooter.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            HighWobbleBackup.robot.shooter.setPower(0);
            HighWobbleBackup.robot.shooter.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
            HighWobbleBackup.robot.intakeRD.setPower(0);
            HighWobbleBackup.robot.deflector.setPower(0);
            HighWobbleBackup.robot.magazine.setPower(0);
        }
    ...
    public static class pickUpNShootRings implements task {
        ...
        public int loop(RobotConstructor robot) {
            ...
            if (state == 0) {
                ...
                if (Math.abs(((DcMotorEx) HighWobbleBackup.robot.shooter).getVelocity()-previousSpeed) >= 80) state = 0;
            } else {
            ...
            if (((DcMotorEx) HighWobbleBackup.robot.intakeRD).getCurrent(CurrentUnit.AMPS) > 1.2) {
                previousSpeed = ((DcMotorEx) HighWobbleBackup.robot.shooter).getVelocity();
    ...
    public void init(int Alliance, int Auto, int Position, int EndPosition) {
        ...
        if (Alliance == 0) { //blue
            ...
            handler.addTask(new HighWobbleBackup.runTillSpeedDown(1000,120));
    ...
    public void loop_init() {
        ...
        if (Alliance == 0) { //blue
            ...
            handler.addTask(new HighWobbleBackup.stopShooter());
            if (numRings == 4) {
            ...
            if (numRings == 4) {
                ...
                handler.addTask(new baseTasks.move(new Point(45,23),0,1,1,5000));
            } else {
