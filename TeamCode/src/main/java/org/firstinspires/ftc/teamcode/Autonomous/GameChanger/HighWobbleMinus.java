...
public class HighWobbleMinus implements autoBase {
    ...
    public HighWobbleMinus(LinearOpMode opMode) {
        this.opMode = opMode;
    ...
    public static class waitForSpeedUp implements task {
        ...
        public int loop(RobotConstructor robot) {
            ...
            if (Math.abs(((DcMotorEx) HighWobbleMinus.robot.shooter).getVelocity()) > robotConstants.shooterSpeed-100) return -1;
    ...
    public static class runTillSpeedDown implements task {
        ...
        public void init(RobotConstructor robot) {
            //set the start time for the timeout and set the motors to run using encoders
            startTime = System.currentTimeMillis();
            HighWobbleMinus.robot.magazine.setPower(0);
            HighWobbleMinus.robot.magazine.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        }

        @Override
        public int loop(RobotConstructor robot) {
            HighWobbleMinus.robot.magazine.setPower(0.75);
            //check if the program has ran for too long, if so, terminate it
            if (System.currentTimeMillis()-startTime >= timeOut) return -2;
            //iterate through each motor
            if (Math.abs(((DcMotorEx) HighWobbleMinus.robot.shooter).getVelocity()) < robotConstants.shooterSpeed-shooterSpeed) {
                HighWobbleMinus.robot.magazine.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                HighWobbleMinus.robot.magazine.setTargetPosition(0);
                HighWobbleMinus.robot.magazine.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                HighWobbleMinus.robot.magazine.setPower(1);
                return -1;
            }
            //return that the program is still running if it has gotten to this point
            return 1;
    ...
    public static class stopShooter implements task {
        ...
        public void init(RobotConstructor robot) {
            //set the start time for the timeout and set the motors to run using encoders
            HighWobbleMinus.robot.shooter.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            HighWobbleMinus.robot.shooter.setPower(0);
            HighWobbleMinus.robot.shooter.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
            HighWobbleMinus.robot.intakeRD.setPower(0);
            HighWobbleMinus.robot.deflector.setPower(0);
            HighWobbleMinus.robot.magazine.setPower(0);
        }
    ...
    public static class pickUpNShootRings implements task {
        ...
        public int loop(RobotConstructor robot) {
            ...
            if (state == 0) {
                ...
                if (Math.abs(((DcMotorEx) HighWobbleMinus.robot.shooter).getVelocity()-previousSpeed) >= 80) state = 0;
            } else {
            ...
            if (((DcMotorEx) HighWobbleMinus.robot.intakeRD).getCurrent(CurrentUnit.AMPS) > 1.2) {
                previousSpeed = ((DcMotorEx) HighWobbleMinus.robot.shooter).getVelocity();
    ...
    public void init(int Alliance, int Auto, int Position, int EndPosition) {
        ...
        if (Alliance == 0) { //blue
            if (Position == 1) { //left
                ...
                handler.addTask(new baseTasks.move(new Point(-17,-15),0, 0.85,1,5000));
            ...
            handler.addTask(new HighWobbleMinus.runTillSpeedDown(1000,120));
    ...
    public void loop_init() {
        ...
        if (Alliance == 0) { //blue
            ...
            if (numRings > 0) {
                ...
                handler.addTask(new HighWobbleMinus.runTillSpeedDown(1000,120));
                if (numRings == 4) {
                    handler.addTask(new baseTasks.servoMovement(robot.CAM, .85, 10));
